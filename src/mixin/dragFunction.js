  ...
  methods: {
    ...
    onMove(option) {
      ...
      if (targetDom.className.includes('free_slot')) {
        if (targetSlotsDataList.length === 0 && draggedElementData.childElement.length === 0) return true;
      }
      ...
      if (targetDom.className.includes('poke_card')) {
        ...
        if (targetCardPosition === 'bottom') {
          if ((Number(draggedElementData.number) + 1 === targetCardNumber) &&
              (draggedElementData.color !== targetCardColor)) return true;
        }
      }
      return false;
    },
